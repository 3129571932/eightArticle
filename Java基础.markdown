...
​		核心线程数，阻塞队列没有满时，最大可以同时运行的线程数量。

##### 	2.maximumPooSize

​		最大线程数，阻塞队列满后，当前线程池可运行的最大线程数变成maxiMumPoolSize
...
​	通```ThreadPoolExecutor```构造函数直接创建（推荐）

​		推荐的方式，因为它允许开发者明确指定线程池的核心参数，对线程池的运行行为有更精细的控制，从		而避免资源耗尽的风险。

​	通过```Executors```工具类创建（不推荐用于生产环境）

​		```FixedThreadPool```固定线程数量的线程池。该线程池中的线程数量始终不变，当有一个新的任务提交		时，线程池中若有空闲的线程，则立即执行，没有则放入队列中，等待空闲线程调用。

​		```SingleThreadExecutor```：只有一个线程的线程池。如果当前线程池没有空闲线程，任务会进入队列		中，按照先入先出的顺序等待调用。

### 线程池的运行原理

​		1.当一个任务通过submit或者execute方法提交到线程池的时候，如果当前线程池中的线程数（包括空闲		线程）小于corePoolSize,则创建一个线程执行该任务。

​		2.如果当前线程池中线程数已经达到corePoolSize，则将任务放入等待队列。

​		3.如果任务不能入队，说明等待队列已经满了，若当前线程池中的线程数小于maximumPoolSize，则创		建一个临时线程（非核心线程）执行该任务，执行完成后变成闲置线程，超过keepAliveTime后就会被回		收。回收的是非核心线程，核心线程一般不会被回收，如果设置allowCoreThreadTimeOut（true）则		超过keepAliveTime后会被回收。
