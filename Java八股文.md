...
​	线程是进程的执行单元，他们共享进程的资源，可以高效的实现并发执行和数据共享。而进程是计算机分配资源的基本单位，每个进程都有自己独立的内存空间和资源，进程之间需要通过IPC进行数据共享和通信。

#### 如何创建线程，有几种方式

​	1.继承thread类，并重写run方法，通过实例对象的start开启线程

​	2.实现runnable接口，并重写run方法，将runnable接口作为参数传递给thread实例对象，通过实例对象的start开启线程
...
#### 什么是线程同步，适合在Java中实现线程同步

#### 线程的生命周期

#### 什么是线程安全，如何保证线程安全

#### Java中的锁有哪些，他们之间的区别是什么

#### volitale

#### 什么是线程通信，Java中有哪些线程通信的机制
